...
final class AddPaymentMethodViewController: UIViewController, AddPaymentMethodPresentable, AddPaymentMethodViewControllable {
    ...
    private lazy var addCardButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(addTapAddCard), for: .touchUpInside)
        return button
    }()
    ...
    @objc func addTapAddCard() {
        if let number = securityTextField.text,
           let cvc = securityTextField.text,
           let expiration = expirationTextField.text {
            listener?.didTapConfirm(with: number, cvc: cvc, expiry: expiration)
        }
    }
